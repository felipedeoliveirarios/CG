...
void cgRotate (d2Vertex *polygon, GLint nVerts, d2Vertex pivPt, GLfloat theta){
    ...
    for (i = 0; i < nVerts; i++) {
        // translacao para o ponto fixo da rotacao
        polygon[i].x -= pivPt.x;
        polygon[i].y -= pivPt.y;

        // rotacao do vertice
        polygon[i].x = (polygon[i].x * cos (theta)) - (polygon[i].y * sin (theta));
        polygon[i].y = (polygon[i].x * sin (theta)) + (polygon[i].y * cos (theta));

        // desfaz a translacao do ponto fixo
        polygon[i].x += pivPt.x;
        polygon[i].y += pivPt.y;
    }
    ...
    // for (i = 0; i < nVerts; i++) {
    //     glVertex2f (printf ("%.2f\n", pivPt.x + ((polygon[i].x - pivPt.x) * cos (theta)) - ((polygon[i].y - pivPt.y) * sin (theta))),
...
d2Vertex hexagon[] = {{20,0},{10,17.3},{-10,17.3},{-20,0},{-10,-17.3},{10,-17.3}};
